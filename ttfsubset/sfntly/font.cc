...
void Font::Builder::ReadHeader(ReadableFontData* fd,
  ...
  for (int32_t table_number = 0;
    ...
    int32_t offset_t = fd->ReadULongAsInt(table_offset + Offset::kTableOffset);
    int32_t length = fd->ReadULongAsInt(table_offset + Offset::kTableLength);
    if (!IsValidHeaderRegion(fd->Size(), offset_t, length))
      continue;

    HeaderPtr table = new Header(tag, checksum, offset_t, length);
    records->insert(table);
  }
}
