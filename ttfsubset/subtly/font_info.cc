...
CALLER_ATTACH FontInfo* FontSourcedInfoBuilder::GetFontInfo() {
  ...
  if (!success) {
    ...
    TLogInfo() << "Error creating character map.";
#endif
    return NULL;
  }
  ...
  if (!success) {
    ...
    TLogInfo() << "Error resolving composite glyphs.";
#endif
    return NULL;
  }
...
FontSourcedInfoBuilder::ResolveCompositeGlyphs(CharacterMap* chars_to_glyph_ids,
  ...
  while (!unresolved_glyph_ids->empty()) {
    ...
    if (glyph_id < 0 || glyph_id > loca_table_->num_glyphs()) {
	  ...
	  char mess[255] = { 0 };
	  sprintf(mess, "%d larger than %d or smaller than 0\n", glyph_id,
		  loca_table_->num_glyphs());
	  TLogInfo() << mess;
#endif
      continue;
    ...
    if (length == 0) {
	  ...
	  char mess[255] = { 0 };
	  sprintf(mess, "Zero length glyph %d\n", glyph_id);
	  TLogInfo() << mess;
#endif
      continue;
    }
    ...
    if (glyph == NULL) {
	  ...
	  char mess[255] = { 0 };
	  sprintf(mess, "GetGlyph returned NULL for %d\n", glyph_id);
	  TLogInfo() << mess;
#endif
      continue;
    }
    ...
    if (glyph->GlyphType() == GlyphType::kComposite) {
      ...
      for (int32_t i = 0; i < num_glyphs; ++i) {
        int32_t glyph_id_t = composite_glyph->GlyphIndex(i);
        if (resolved_glyph_ids->find(GlyphId(glyph_id_t, -1))
            == resolved_glyph_ids->end()) {
          unresolved_glyph_ids->insert(glyph_id_t);
        }
      }
    }
  }
