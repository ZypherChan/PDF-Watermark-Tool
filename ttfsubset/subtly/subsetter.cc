...
CALLER_ATTACH Font* Subsetter::Subset() {
  ...
  uint32_t subset_size = font_info->chars_to_glyph_ids()->size();
  if ( subset_size == 0)
  {
      fprintf(stderr,
          "Couldn't create font info.Subset chars_to_glyph_ids is empty.\n");
      return NULL;
  }
  uint32_t total_size = predicate_->size();
  if (subset_size < total_size)
  {
      fprintf(stderr,
          "Couldn't create font info.Subset chars_to_glyph_ids size less than subset size.\n");
      // return NULL;
  ...
  table_blacklist->insert(Tag::EBDT);
  table_blacklist->insert(Tag::EBLC);
...
bool Subsetter::HasMissingGlyphs() const
{
